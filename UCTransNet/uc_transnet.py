...
class Attention_org(nn.Module):
    def __init__(self, vis=False,channel_num=[64, 128, 256, 512]):
...
class Block_ViT(nn.Module):
    def __init__(self,  vis=False, channel_num=[64, 128, 256, 512]):
        ...
        self.channel_attn = Attention_org(vis=False, channel_num=[64, 128, 256, 512])
...
class Encoder(nn.Module):
    def __init__(self, vis, channel_num=[64, 128, 256, 512]):
        ...
        for _ in range(4):
            layer = Block_ViT(vis=False, channel_num=[64, 128, 256, 512])
...
class ChannelTransformer(nn.Module):
    def __init__(self,  vis, img_size, channel_num=[64, 128, 256, 512], patchSize=[32, 16, 8, 4]):
        ...
        self.encoder = Encoder(vis, [64, 128, 256, 512])
...
class UC_TransNet(nn.Module):

    ## Conv 3 + MLP 2 + shifted MLP
    
    def __init__(self,  num_classes, input_channels=3, deep_supervision=False,img_size=384, patch_size=16, in_chans=3,  embed_dims=[32, 64, 128, 512],
                 n_channels=3, vis=False,
        ...
        self.mtc = ChannelTransformer(vis=False, img_size=384,
